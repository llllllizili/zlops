        ...
        >
          ...
          <el-table-column label="当前状态" min-width="100">
            <template slot-scope="scope">
              <el-tag
                v-if="scope.row.act_state == 0"
                label="草稿中"
                value="scope.row.act_state"
                >草稿中</el-tag
              >
              <el-tag
                v-else-if="scope.row.act_state == 1"
                label="进行中"
                value="scope.row.act_state"
                >进行中</el-tag
              >
              <el-tag
                v-else-if="scope.row.act_state == 2"
                label="被退回"
                value="scope.row.act_state"
                >被退回</el-tag
              >
              <el-tag
                v-else-if="scope.row.act_state == 3"
                label="被撤回"
                value="scope.row.act_state"
                >被撤回</el-tag
              >
              <el-tag
                v-else-if="scope.row.act_state == 4"
                label="已完成"
                value="scope.row.act_state"
                >已完成</el-tag
              >
              <el-tag
                v-else-if="scope.row.act_state == 5"
                label="已关闭"
                value="scope.row.act_state"
                >已关闭</el-tag
              >
          ...
          <el-table-column align="center" label="操作">
            <template slot-scope="scope">
              ...
              <el-link
                ...
                >撤回</el-link
              >
              <el-link type="primary" @click="handleDetails(scope)"
                >详情</el-link
              >
              <el-link type="success" @click="handleDelete(scope)"
                >删除</el-link
              >
              <el-link type="success" @click="handlePicture(scope)"
                >流程</el-link
              >
              <el-link type="success" @click="handleLogs(scope)"
                >日志</el-link
              >
            </template>
          </el-table-column>
        ...
        >
          ...
          <el-table-column align="center" label="操作">
            <template slot-scope="scope" v-if="scope.row.state_">
              <el-link
                v-if="
                  scope.row.state_.enable_retreat && scope.row.state_.type !== 1
                "
                type="danger"
                @click="handleClose(scope, '1')"
                >撤回</el-link
          ...
          <el-table-column align="center" label="操作">
            <template slot-scope="scope">
              <el-link type="primary" @click="handleDetails(scope)"
                >详情</el-link
              >
              <el-link type="danger" @click="handlePicture(scope)"
                >查看流程图</el-link
              >
              <el-link type="success" @click="handleLogs(scope)"
                >工单日志</el-link
    ...
    <div class="svgMark" v-if="dialogVisible" @click="closeMark">
      <div class="svgWrapper">
        ...
        <div style="width: 90%; margin: auto">
          <svg height="1000" id="mySvg" style="width: 100% !important"></svg>
        </div>
      </div>
    </div>
    ...
    <el-dialog :visible.sync="limitedFlowLogs" title="工单日志">
      ...
      >
        ...
        <el-table-column label="操作人" min-width="100">
          <template slot-scope="scope" v-if="scope.row.participant_">{{
...
export default {
  ...
  methods: {
    ...
    resetFilter() {
      (this.pageForm.workflow = ""), this.getList();
    },
    handleCreate() {
                      ...
                      transitionList.forEach((transitions) => {
                        ...
                        if (transition0.condition_expression.length > 0) {
                          ...
                          let condition_expression =
                            transition0.condition_expression;
                          condition_expression.forEach(
                            (condition_expression0) => {
                              g.setEdge(
                                transition0.source_state_.id + 100000,
    ...
    handleDetail(scope) {
      this.$router.push({
        name: "ticketHandle",
        params: { ticketId: scope.row.id, workflow: scope.row.workflow },
      });
    },
    //工单详情
    handleDetails(scope) {
