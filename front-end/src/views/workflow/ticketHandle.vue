  ...
  <div class="app-container">
          ...
          >
            ...
            <el-col :span="12">
              <el-form-item label="工作流" style="margin-bottom: 0">
                <span v-if="ticketDetail.workflow_">{{
            ...
            <el-row style="display: flex; flex-wrap: wrap; width: 100%">
              <el-col
                v-for="item in ticketDetail.ticket_data_"
                :key="item.id"
                :span="12"
                v-show="!item.is_hidden"
              >
                <el-form-item
                  v-if="item.field_state == '1'"
          ...
          >
            <div>
              <template v-if="ticketDetail.state_">
                ...
                <el-button
                  v-if="
                    ticketDetail.act_state == 1 &&
                    ticketDetail.state_.type === 0 &&
                    !ticketDetail.in_add_node
                  "
                  type="primary"
                  @click="handleAdd"
                  >加签</el-button
                >
                <el-button
                  v-if="ticketDetail.state_.type === 1"
                  type="primary"
                  @click="handleClick('2')"
                  >关闭</el-button
                >
                <el-button
                  v-if="
                    ticketDetail.state_.enable_retreat &&
                    userId == ticketDetail.create_by &&
                    ticketDetail.state_.type !== 1
                  "
                  type="danger"
                  @click="handleClick('1')"
                  >撤回</el-button
    ...
    <el-dialog :visible.sync="limitedHandle" :title="handleTitle">
      ...
      <div style="text-align: center">
        <el-button class="filter-item" type="" @click="retreatCancel"
          >取消</el-button
        >
    ...
    <el-dialog :visible.sync="limitedAdd" title="加签">
      ...
      <div style="text-align: center">
        <el-button class="filter-item" type="" @click="addCancel"
          >取消</el-button
        >
        <el-button class="filter-item" type="primary" @click="addSubmit"
          >确定</el-button
        >
      </div>
    </el-dialog>
            ...
            if (res.data) {
              ...
              getTicketDetail(ticketId).then((res) => {
                if (res.data) {
                  ...
                  for (
                    let i = 0;
                    i < that.ticketDetail.ticket_data_.length;
                    i++
                  ) {
                    if (
                      that.ticketDetail.ticket_data_[i].label === "sys_user"
                    ) {
                      that.ticketDetail.ticket_data_[i].field_choice =
                        that.staffs;
                    }
                    if (
                      that.ticketDetail.ticket_data_[i].label === "deptSelect"
                    ) {
                      that.ticketDetail.ticket_data_[i].field_choice =
                        that.orgList;
                    }
                  ...
                  if (state_fields !== {}) {
                    for (let labe in state_fields) {
                      for (
                        let j = 0;
                        j < that.ticketDetail.ticket_data_.length;
                        j++
                      ) {
                        if (
