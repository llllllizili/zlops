...
from apps.wf.serializers import (
    CustomFieldCreateUpdateSerializer,
    CustomFieldSerializer,
    StateSerializer,
    TicketAddNodeEndSerializer,
    TicketAddNodeSerializer,
    TicketCloseSerializer,
    TicketCreateSerializer,
    TicketDestorySerializer,
    TicketFlowSerializer,
    TicketFlowSimpleSerializer,
    TicketHandleSerializer,
    TicketRetreatSerializer,
    TicketSerializer,
...
from apps.system.mixins import (
...
class WorkflowViewSet(CreateUpdateModelAMixin, ModelViewSet):
    ...
    def states(self, request, pk=None):
        ...
        serializer = self.serializer_class(
            instance=WfService.get_worlflow_states(wf), many=True
        )
    ...
    def transitions(self, request, pk=None):
        ...
        wf = self.get_object()
        serializer = self.serializer_class(
            instance=WfService.get_workflow_transitions(wf), many=True
        )
        return Response(serializer.data)
    ...
    def customfields(self, request, pk=None):
        ...
        wf = self.get_object()
        serializer = self.serializer_class(
            instance=CustomField.objects.filter(workflow=wf, is_deleted=False).order_by(
                "sort"
            ),
            many=True,
        )
    ...
    def init(self, request, pk=None):
        ...
        field_list = CustomFieldSerializer(
            instance=WfService.get_workflow_custom_fields(wf), many=True
        ).data
        for i in field_list:
...
class TicketViewSet(
    ...
    def duty_agg(self, request, pk=None):
        ...
        queryset = Ticket.objects.filter(
            participant__contains=request.user.id, is_deleted=False
        ).exclude(
            act_state__in=[
                Ticket.TICKET_ACT_STATE_FINISH,
                Ticket.TICKET_ACT_STATE_CLOSED,
            ]
        )
        ret["total_count"] = queryset.count()
        ret["details"] = list(
            queryset.values("workflow", "workflow__name").annotate(
    ...
    @action(
        ...
        detail=True,
        perms_map={"post": "*"},
        serializer_class=TicketAddNodeSerializer,
    )
    ...
    @action(
        ...
        detail=True,
        perms_map={"post": "*"},
        serializer_class=TicketAddNodeEndSerializer,
    )
    ...
    def close(self, request, pk=None):
        ...
        ticket = self.get_object()
        if (
            ticket.state.type == State.STATE_TYPE_START
            and ticket.create_by == request.user
        ):
