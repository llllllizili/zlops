...
class Workflow(CommonAModel):
    ...
    key = models.CharField(
        "工作流标识", unique=True, max_length=20, null=True, blank=True
    )
    ...
    view_permission_check = models.BooleanField(
        "查看权限校验",
        default=True,
        help_text="开启后，只允许工单的关联人(创建人、曾经的处理人)有权限查看工单",
    )
    ...
    display_form_str = models.JSONField(
        "展现表单字段",
        default=list,
        blank=True,
    ...
    content_template = models.CharField(
        "内容模板",
        max_length=1000,
        default="标题:{title}, 创建时间:{create_time}",
        null=True,
        blank=True,
        help_text="工单字段的值可以作为参数写到模板中，格式如：标题:{title}, 创建时间:{create_time}",
    )
...
class State(CommonAModel):
    ...
    STATE_TYPE_START = 1
    STATE_TYPE_END = 2
    type_choices = ((0, "普通"), (STATE_TYPE_START, "开始"), (STATE_TYPE_END, "结束"))
    ...
    STATE_DISTRIBUTE_TYPE_DIRECT = (
        2  # 直接处理(当前为多人的情况，都可以处理，而不需要先接单)
    )
    ...
    workflow = models.ForeignKey(
        Workflow, on_delete=models.CASCADE, verbose_name="所属工作流"
    )
    ...
    sort = models.IntegerField(
        "状态顺序",
        default=0,
        help_text="用于工单步骤接口时，step上状态的顺序(因为存在网状情况，所以需要人为设定顺序),值越小越靠前",
    )
    ...
    participant_type = models.IntegerField(
        "参与者类型",
        choices=state_participanttype_choices,
        default=1,
        blank=True,
    ...
    state_fields = models.JSONField(
        "表单字段",
        default=dict,
        help_text='json格式字典存储,包括读写属性1：只读，2：必填，3：可选, 4:隐藏 示例：{"create_time":1,"title":2, "sn":1}, 内置特殊字段participant_info.participant_name:当前处理人信息(部门名称、角色名称)，state.state_name:当前状态的状态名,workflow.workflow_name:工作流名称',
    )  # json格式存储,包括读写属性1：只读，2：必填，3：可选，4：不显示, 字典的字典
    distribute_type = models.IntegerField(
        "分配方式",
        default=1,
        choices=state_distribute_choices,
    ...
    filter_policy = models.IntegerField(
        "参与人过滤策略", default=0, choices=state_filter_choices
    )
    participant_cc = models.JSONField(
        "抄送给", default=list, blank=True, help_text="抄送给(userid列表)"
    )
...
class Transition(CommonAModel):
    ...
    attribute_type_choices = ((1, "同意"), (2, "拒绝"), (3, "其他"))
    ...
    timer = models.IntegerField(
        "定时器(单位秒)",
        default=0,
        help_text="单位秒。处于源状态X秒后如果状态都没有过变化则自动流转到目标状态。设置时间有效",
    )
    source_state = models.ForeignKey(
        State,
        on_delete=models.CASCADE,
        verbose_name="源状态",
        related_name="sstate_transition",
    )
    destination_state = models.ForeignKey(
        State,
        on_delete=models.CASCADE,
        verbose_name="目的状态",
        related_name="dstate_transition",
    )
...
class CustomField(CommonAModel):
    ...
    field_key = models.CharField(
        "字段标识",
        max_length=50,
        help_text="字段类型请尽量特殊，避免与系统中关键字冲突",
    )
    ...
    boolean_field_display = models.JSONField(
        "布尔类型显示名",
        default=dict,
        blank=True,
    ...
    is_hidden = models.BooleanField(
        "是否隐藏", default=False, help_text="可用于携带不需要用户查看的字段信息"
    )
    group = models.CharField("字段分组", max_length=100, null=True, blank=True)
    parent = models.ForeignKey(
        "self", verbose_name="父字段", on_delete=models.SET_NULL, null=True, blank=True
    )
...
class Ticket(CommonBModel):
    ...
    title = models.CharField(
        "标题", max_length=500, null=True, blank=True, help_text="工单标题"
    )
    workflow = models.ForeignKey(
        Workflow, on_delete=models.CASCADE, verbose_name="关联工作流"
    )
    ...
    state = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        verbose_name="当前状态",
        related_name="ticket_state",
    )
    parent = models.ForeignKey(
        "self", null=True, blank=True, on_delete=models.CASCADE, verbose_name="父工单"
    )
    parent_state = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        verbose_name="父工单状态",
        related_name="ticket_parent_state",
    )
    ticket_data = models.JSONField(
        "工单数据", default=dict, help_text="工单自定义字段内容"
    )
    in_add_node = models.BooleanField(
        "加签状态中", default=False, help_text="是否处于加签状态下"
    )
    add_node_man = models.ForeignKey(
        User,
        verbose_name="加签人",
    ...
    participant_type = models.IntegerField(
        "当前处理人类型",
        default=0,
        help_text="0.无处理人,1.个人,2.多人",
        choices=State.state_participanttype_choices,
    )
    participant = models.JSONField(
        "当前处理人",
        default=list,
        blank=True,
        help_text="可以为空(无处理人的情况，如结束状态)、userid、userid列表",
    )
    act_state = models.IntegerField(
        "进行状态", default=1, help_text="当前工单的进行状态", choices=act_state_choices
    )
    multi_all_person = models.JSONField(
        "全部处理的结果",
        default=dict,
        blank=True,
...
class TicketFlow(BaseModel):
    ...
    ticket = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        verbose_name="关联工单",
        related_name="ticketflow_ticket",
    )
    transition = models.ForeignKey(
        Transition,
        verbose_name="流转id",
        help_text="与worklow.Transition关联， 为空时表示认为干预的操作",
        on_delete=models.CASCADE,
        null=True,
        blank=True,
    )
    ...
    participant = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="ticketflow_participant",
    )
    participant_str = models.CharField(
        "处理人",
        max_length=200,
        null=True,
        blank=True,
        help_text="非人工处理的处理人相关信息",
    )
    state = models.ForeignKey(
        State, verbose_name="当前状态", default=0, blank=True, on_delete=models.CASCADE
    )
    ticket_data = models.JSONField(
        "工单数据",
        default=dict,
        blank=True,
        help_text="可以用于记录当前表单数据，json格式",
    )
    intervene_type = models.IntegerField(
        "干预类型",
        default=0,
        help_text="流转类型",
        choices=Transition.intervene_type_choices,
    )
    participant_cc = models.JSONField(
        "抄送给", default=list, blank=True, help_text="抄送给(userid列表)"
    )
