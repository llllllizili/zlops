...
class WfService(object):
    ...
    def get_worlflow_states(workflow: Workflow):
        ...
        获取工作流状态列表
        """
        return State.objects.filter(workflow=workflow, is_deleted=False).order_by(
            "sort"
        )
    ...
    def get_workflow_start_state(workflow: Workflow):
        ...
        """
        try:
            wf_state_obj = State.objects.get(
                workflow=workflow, type=State.STATE_TYPE_START, is_deleted=False
            )
            return wf_state_obj
        except:
    ...
    def get_workflow_end_state(workflow: Workflow):
        ...
        try:
            wf_state_obj = State.objects.get(
                workflow=workflow, type=State.STATE_TYPE_END, is_deleted=False
            )
            return wf_state_obj
        except:
            raise Exception("工作流状态配置错误")
    ...
    def get_workflow_custom_fields(workflow: Workflow):
        ...
        获取工单字段
        """
        return CustomField.objects.filter(is_deleted=False, workflow=workflow).order_by(
            "sort"
        )
    ...
    def get_workflow_custom_fields_list(workflow: Workflow):
        ...
        """
        return list(
            CustomField.objects.filter(is_deleted=False, workflow=workflow)
            .order_by("sort")
    ...
    def get_ticket_sn(cls, workflow: Workflow):
        ...
        ticket_day_count_new = (
            Ticket.objects.filter(
                create_time__gte=today, create_time__lte=next_day, workflow=workflow
            ).count()
            + 1
        )
        return "%s_%04d%02d%02d%04d" % (
            workflow.sn_prefix,
            now.year,
            now.month,
            now.day,
    ...
    @classmethod
    def get_next_state_by_transition_and_ticket_info(
        cls, ticket: Ticket, transition: Transition, new_ticket_data: dict = {}
    ) -> object:
        """
        获取下个节点状态
        """
    ...
    @classmethod
    def get_ticket_state_participant_info(
        cls,
        state: State,
        ticket: Ticket,
        ...
        if destination_participant_type == State.PARTICIPANT_TYPE_FIELD:
            ...
            destination_participant = list(
                User.objects.filter(dept__in=destination_participant).values_list(
                    "id", flat=True
                )
            )
        ...
        return dict(
            destination_participant_type=destination_participant_type,
    ...
    def ticket_handle_permission_check(cls, ticket: Ticket, user: User) -> dict:
        ...
        if participant_type == State.PARTICIPANT_TYPE_PERSONAL:
            ...
            State.PARTICIPANT_TYPE_MULTI,
            State.PARTICIPANT_TYPE_DEPT,
            State.PARTICIPANT_TYPE_ROLE,
        ]:
        ...
        if current_participant_count == 1:
            ...
            and state.distribute_type == State.STATE_DISTRIBUTE_TYPE_ACTIVE
        ):
            if user.id not in participant:
                return dict(permission=False, msg="非当前处理人", need_accept=False)
    ...
    def handle_ticket(
        ...
        destination_state = cls.get_next_state_by_transition_and_ticket_info(
            ticket, transition, new_ticket_data
        )
        ...
        if multi_all_person:
            ...
            if WfService.check_dict_has_all_same_value(multi_all_person):
                ...
                destination_participant_type = participant_info.get(
                    "destination_participant_type", 0
                )
                destination_participant = participant_info.get(
                    "destination_participant", 0
                )
                multi_all_person = {}
            else:
        ...
        if created:
            if source_state.participant_cc:
                TicketFlow.objects.create(
                    ...
                    state=source_state,
                    participant_type=0,
                    intervene_type=Transition.TRANSITION_INTERVENE_TYPE_CC,
                    participant=None,
                    participant_cc=source_state.participant_cc,
                )
        ...
        if destination_state.participant_cc:
            TicketFlow.objects.create(
                ...
                intervene_type=Transition.TRANSITION_INTERVENE_TYPE_CC,
                participant=None,
                participant_cc=destination_state.participant_cc,
            )
        # # 如果目标状态是脚本则执行
    ...
    def task_ticket(cls, ticket: Ticket):
        ...
        # 抄送-邮件发送
        if last_log.intervene_type == Transition.TRANSITION_INTERVENE_TYPE_CC:
            ticket.participant_type = 2
            ticket.participant = last_log.participant_cc
            Thread(target=send_ticket_notice, args=(ticket,"抄送提醒"), daemon=True).start()
...
def send_ticket_notice(ticket: Ticket, subject=None, message=None):
    """
    发送通知
    """
    # params = {"workflow": ticket.workflow.name, "state": ticket.state.name}
    sub = subject if subject is not None else '工单提醒'
