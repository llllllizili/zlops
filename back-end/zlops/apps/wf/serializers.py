...
class TransitionSerializer(serializers.ModelSerializer):
    ...
    destination_state_ = StateSimpleSerializer(
        source="destination_state", read_only=True
    )
...
class TicketCreateSerializer(serializers.ModelSerializer):
    transition = serializers.PrimaryKeyRelatedField(
        queryset=Transition.objects.all(), write_only=True
    )
            ...
            if i["field_value"]:
                if "sys_user" in i["label"]:
                    if isinstance(i["field_value"], list):
                        i["field_display"] = ",".join(
                            list(
                                User.objects.filter(
                                    id__in=i["field_value"]
                    ...
                    if isinstance(i["field_value"], list):
                        ...
                        i["field_display"] = Organization.objects.get(
                            id=i["field_value"]
                        ).name
...
class TicketHandleSerializer(serializers.Serializer):
    transition = serializers.PrimaryKeyRelatedField(
        queryset=Transition.objects.all(), label="流转id"
    )
    ticket_data = serializers.JSONField(label="表单数据json")
    suggestion = serializers.CharField(
        label="处理意见", required=False, allow_blank=True
    )
...
class TicketAddNodeSerializer(serializers.Serializer):
    ...
    toadd_user = serializers.PrimaryKeyRelatedField(
        queryset=User.objects.all(), label="发送给谁去加签"
    )
...
class TicketDestorySerializer(serializers.Serializer):
    ...
    create_by = serializers.PrimaryKeyRelatedField(
        queryset=User.objects.all(), label="工单创建人"
    )
